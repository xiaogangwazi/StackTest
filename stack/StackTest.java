...
public class StackTest {
    public static void main(String[] args) {
        ...
        while (action != -1) {
            String action1 = stack.action(action);
    ...
    static class Stack {
        ...
        public String action(int action) {
            if (action==1&&step < point-1) {
                return stack[++step];
            } else if(action==1&&step>=point-1){
                System.out.println("不能前进了");

            }else if(action==0&&step>0){

                return stack[--step];
            }else if(action==0&&step<=0){
                System.out.println("不能再后退了");
